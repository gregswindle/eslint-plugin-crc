...
const {first, get, isEmpty, replace} = require("lodash");
...
class CrcResponsibility {
  ...
  static descriptionFromContext (context) {
    ...
    return replace(description, /\s+/gim, " ").trim();
  }
...
const getComments = (crcClass) => {
  ...
  return comments.map((comment) => doctrine.parse(comment.value, doctrineOpts.parse));
};
