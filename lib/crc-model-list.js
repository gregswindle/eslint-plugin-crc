 ...
 (function() {
     ...
     const crcModelPrototypeVisitor = {
          ...
          * @returns {CrcModel|Error} The `CrcModel`'s prototype or an `Error` if a prototype does not exist for it.'
          */
         getPrototypeByNewExpression: function (crcModel, crcModelList) {
             let crcPrototype, FIRST_DECLARATION;
             FIRST_DECLARATION = 0;
             try {
                 let predicate = libCrc.templates.getPrototypeOfNewExpression;
                 predicate.declarations[FIRST_DECLARATION].id.name = crcModel.name;
                 const newExpression = _.find(crcModelList.ast.body, predicate);
                 const name = _.get(
                    _.first(newExpression.declarations),
                    'init.callee.name'
                  );
                 crcPrototype = crcModelList.find({name: name}).superClass;
             }
             catch (err) {
                 crcPrototype = err;
             }
     ...
     class CrcModelList {
       ...
       */
         ...
         getPrototypeOf(crcModel) {
             ...
             return isInvalidObject(prototype) ? undefined : prototype;
         }
