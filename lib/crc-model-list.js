 ...
 const getPrototypeOfExpressStatementPredicateJson = require('./templates/get-prototype-of-expression-statement');
 const getPrototypeOfMemberExpressionPredicateJson = require('./templates/get-prototype-of-member-expression');
 ...
 (function() {
      ...
      *
      * @returns {CrcModel|undefind} The `CrcModel`'s prototype or undefined if a prototype does not exist for it.'
      */
     const getPrototypeByObjectCreate = function (crcModel, crcModelList) {
         // Definition 1: Object.create
         let predicate = getPrototypeOfExpressStatementPredicateJson;
         predicate.expression.left.object.name = crcModel.name;
         const expressionStatement = _.find(crcModelList.ast.body, predicate);
         predicate = getPrototypeOfMemberExpressionPredicateJson;
         const memberExpression = _
            .find(expressionStatement.expression.right.arguments, predicate);
         const name = _.get(memberExpression, 'object.name');
         return crcModelList.find({name: name});
     };
   ...
   */
     class CrcModelList {
       ...
       */
          ...
          *
          * @see https://github.com/gregswindle/eslint-plugin-crc/wiki/Strategies-for-prototype-detection
          * @returns {CrcModel|undefined} The prototype of the crcModel, or undefined if
          * not found.
          */
         getPrototypeOf(crcModel) {
             let prototype = getPrototypeByObjectCreate(crcModel, this);
