...
(() => {
   ...
   *
   * @example
   * const messages = renderMessages(result.messages, 0, filePath));
   * @param {Array} messages - Messages.
   * @param {int} parentIndex - Index of the parent HTML row.
   * @param {string} filePath - The path to the source code.
   * @returns {string} HTML (table rows) describing the messages.
   * @private
   */
  function renderMessages (messages, parentIndex, filePath) {
  /**
  ...
  function renderResults (results) {
    ...
    }) + renderMessages(result.messages, index, result.filePath)).join("\n");
  }
